...
export async function objectCreationHandler(req: Request): Promise<void> {
  ...
  for (const record of webhook.Records) {
    ...
    console.log(`[${correlationId}] Processing object creation: ${bucket}/${decodedKey}`);

    try {
      const parts = decodedKey.split('/');
      const fileId = parts[parts.length - 1];
      
      console.log(`[${correlationId}] Extracted fileId: ${fileId}`);
      ...
      if (result.count === 0) {
        console.warn(`[${correlationId}] No pending upload found for fileId: ${fileId}`);
      } else {
        console.log(`[${correlationId}] Successfully updated ${result.count} record(s) to AVAILABLE`);
      }
    } catch (error) {
      console.error(`[${correlationId}] Error updating database for ${decodedKey}:`, error);
      throw error;
    }
  }
}
...
export async function objectDeletionHandler(req: Request): Promise<void> {
  ...
  for (const record of webhook.Records) {
    ...
    // Decode the URL-encoded key
    const decodedKey = decodeURIComponent(key);
    console.log(`[${correlationId}] Processing object deletion: ${decodedKey}`);
    ...
    try {
      ...
      if (result.count === 0) {
        console.warn(`[${correlationId}] No file record found for deletion: ${fileId}`);
      } else {
        console.log(`[${correlationId}] Successfully marked ${result.count} record(s) as DELETED`);
      }
    } catch (error) {
      console.error(`[${correlationId}] Error processing deletion for ${decodedKey}:`, error);
      throw error;
    }
  }
}
...
export const handleMinIOWebhook = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (hasCreationEvents) {
      await objectCreationHandler(req);
    }
    if (hasDeletionEvents) {
      await objectDeletionHandler(req);
    }
